...
`allowed_moving_actors=None` disables explicit protection; `[]` preserves no actor changes; a list permits those actor names. GEN/BENCH combine batch lists and restore disallowed actor states after tool updates. Any `None` disables batch protection. Incompatible layouts/names skip protection with a warning.
