...
If you need the full execution model, registry behavior, `redo`, multi-API inheritance, or advanced logging patterns, continue with [Create a Tool (Detailed)](../../deep-dive/create-tools.md).

:::info
This page complements the main [Create your custom scenario in 5 minutes](../create-scenarios.md) tutorial.
Use it as a drill-down on the tool step, not as a separate authoring flow.
:::
...
At the end of the execution, MAGMA calls your `verifier(...)`, which must return a `ToolResult`. Generally you define the verifier inside the tool and pass it inside the `ToolExecution`.
...
- [Create a Tool (Detailed)](../../deep-dive/create-tools.md) if you need `redo`, logs, or multiple tool variants
